        ...
        : Effect
    {
            ...
            Rectangle[] rois, int startIndex, int length)
        {
            ...
            for (int ri = startIndex; ri < startIndex + length; ++ri)
            {
                ...
                {
                    //ColorPixelBase* dstPixel = dstArgs.Surface.GetPointAddressUnchecked(roi.Left, y);
                    ...
                    for (int x = roi.Left; x < roi.Right; ++x)
                    {
                        ...
                        for (int fy = fyStart; fy < fyEnd; ++fy)
                        {
                            ...
                            for (int fx = fxStart; fx < fxEnd; ++fx)
                            {
                                int srcX = fx + srcX1;
                                int weight = wRow[fx];
                                ColorPixelBase c = srcArgs.Surface.GetPoint(srcX1+xS , srcY);
                                

                                alphaFactor += weight;
                                weight = weight * (c.alpha  + (c.alpha  >> 7));
                                colorFactor += weight;
                                weight >>= 8;
                        ...
                        dstArgs.Surface.SetPoint(x,y, srcArgs.Surface.ColorPixelBase .FromBgra((byte)blueSum, (byte)greenSum, (byte)redSum, (byte)alphaSum));
                    }
                }
            }
        }

        public ConvolutionFilterEffect(string name, Image image, EffectFlags flags)
