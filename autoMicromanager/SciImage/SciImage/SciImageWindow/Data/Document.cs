...
namespace SciImage
{
          ...
          ICloneable
    {
        ...
        [NonSerialized]
        private SciImage.Threading.ThreadPool threadPool = new SciImage.Threading.ThreadPool();
        ...
        /// <summary>
        /// Clears a portion of a RGB32_Surface to transparent.
        /// </summary>
        /// <param name="RGB32_Surface">The RGB32_Surface to partially clear</param>
        ...
        private unsafe void ClearBackground(Surface surface, Rectangle roi)
        {
            ...
            surface.Clear(roi, surface.ColorPixelBase.WhiteColor());
        ...
        public void Render(RenderArgs args, Rectangle roi, bool clearBackground)
        {
            ...
            if (clearBackground)
            {
                ...
                // Special case: if the first blayer is a visible BitmapLayer with full opacity using 
        ...
        public void OnDeserialization(object sender)
        {
            ...
            this.threadPool = new SciImage.Threading.ThreadPool();
            SetupEvents();
            Dirty = true;
        }
        ...
        /*
         BitmapLayer blayer = Layer.CreateBackgroundLayer(image.Width, image.Height);
                    ...
                    unsafe
                    {
                        ...
                        try
                        {
                            for (int y = 0; y < bData.Height; ++y)
                            {
                                ...
                                ColorPixelBase* dstPtr = blayer.Surface.GetRowAddress(y);
                        ...
                        try
                        {
                            for (int y = 0; y < bData.Height; ++y)
                            {
                                ...
                                for (int x = 0; x < bData.Width; ++x)
                                {
                                    ...
                                    *dstPtr = ColorPixelBase.FromBgra(b, g, r, a);
                ...
                {
                    using (RenderArgs args = new RenderArgs(blayer.Surface))
                    {
        ...
        public static Document FromLayer(Layer layer)
        {
            Document document = new Document(layer.Width, layer.Height);
            // Transfer metadata
            ...
            document.DpuX = 100;// image.HorizontalResolution;
            document.DpuY = 100;//image.VerticalResolution;
        ...
        public static Document FromImage(Image image)
        {
            ...
            {
                layer = null;// IntensityLayer.FromImage(image);
            }
            else
            {
                layer = BitmapLayer.FromImage(image);
            }
            // Transfer metadata
        ...
        /// </summary>
        /// <param name="image">The Image to make a copy of that will be the first blayer ("Background") in the document.</param>
        public static Document FromImage(int width, int height, int RawStride, int RawBPP, int RawlSize, IntPtr RawData, long MaxContrast, long MinContrast)
        {
            if (RawData  == null)
            {
                throw new ArgumentNullException("image");
            ...
            Layer layer=null;
            if (RawBPP == 16 | RawBPP == 8)
            {
                layer = null;// IntensityLayer.FromImage(width, height, RawStride, RawBPP, RawlSize, RawData, MaxContrast, MinContrast);
            }
            else
                layer = null;// BitmapLayer.FromImage(width, height, RawStride, RawBPP, RawlSize, RawData, MaxContrast, MinContrast);
        ...
        public static Document FromStream(Stream stream)
        {
            ...
            sfb.AddAssembly(Assembly.GetExecutingAssembly());     // first try SciImage.Data.dll
            sfb.AddAssembly(typeof(Utility).Assembly);            // second, try SciImage.Core.dll
            sfb.AddAssembly(typeof(SystemLayer.Memory).Assembly); // third, try SciImage.SystemLayer.dll
        ...
        /// Layers.Count*Width*Height*sizeof(ColorPixelBase). The final number will actually be higher 
        ...
        public void Flatten(Surface dst)
        {
            ...
            dst.Clear(dst.ColorPixelBase.WhiteColor().NewAlpha(0));
        ...
        /// "Flattened" means it is one blayer that is simply a bitmap of
        ...
        public Document Flatten()
        {
            ...
            BitmapLayer layer =(BitmapLayer) Layer.CreateBackgroundLayer(width, height,new ColorBgra());
