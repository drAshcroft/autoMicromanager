...
namespace SciImage.Tools
{
        ...
        : Tool
    {
        ...
        private string statusBarTextFormat = "Anchor at: {0}{1}, {2}{3}";
        ...
        private SciImage.Threading.ThreadPool threadPool;
        ...
        protected override void OnActivate()
        {
            ...
            this.moveNub.Shape = SciImage.MoveNubRenderer.MoveNubShape.Compass;
        ...
        private void DrawText(Surface dst, Font textFont, string text, Point pt, Size measuredSize, bool antiAliasing, Brush brush)
        {
            ...
            using (Surface surface = new Surface(8, 8,dst.ColorPixelBase ))
            {
            ...
            using (RenderArgs renderArgs = new RenderArgs(this.ScratchSurface))
            {
                ...
                if (dst.IsColumnVisible(pt.X + skipX))
                {
                    for (int y = pt.Y; y < pt.Y + measuredSize.Height; ++y)
                    {
                        ...
                        //ColorPixelBase *dstPtr = dst.GetPointAddressUnchecked(pt.X + skipX, y);
                        

                        for (int x = pt.X + skipX; x < xEnd; ++x)
                        {
                            //ColorPixelBase* srcPtr = ScratchSurface.GetPointAddress(pt.X + skipX, y);
                            //ColorPixelBase* brushPtr = brush8x8.GetRowAddressUnchecked(y & 7);

                            ColorPixelBase srcPixel = ScratchSurface.GetPoint(x,y);
                            ColorPixelBase dstPixel = dst.GetPoint(x,y);
                            ColorPixelBase brushPixel = brush8x8.GetPoint(x & 7,y & 7);

                            int alpha = ((255 - srcPixel[2] ) * brushPixel.alpha ) / 255; // we could use srcPixel.R, .G, or .B -- the choice here is arbitrary
                            brushPixel.alpha  = (byte)alpha;

                            if (srcPixel[2] == 255) // could use R, G, or B -- arbitrary choice
                            {
                            ...
                            else if (alpha == 255 || !blending)
                            {
                                ...
                                dst.SetPoint(x,y, brushPixel);
                            }
                            else
                            {
                                // do expensive blending
                                dst.SetPoint(x,y, UserBlendOps.NormalBlendOp.ApplyStatic(dstPixel, brushPixel));
                            }
        ...
        public TextTool(DocumentWorkspace documentWorkspace)
            : base(documentWorkspace,
                   ...
                   "Text Stamp",
                   "Click to locate text and then begin to type",
                   't',
                   false,
