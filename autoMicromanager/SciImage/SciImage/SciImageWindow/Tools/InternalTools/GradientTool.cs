...
namespace SciImage.Tools
{
        ...
        : Tool
    {
        ...
        private string helpTextInitial = "Click and drag to set an gradient";
        private string helpTextWhileAdjustingFormat = "Origin {0}{1} x {2}{3}, by length: {4} {5}, and an angle of {6}°. ";
        private string helpTextAdjustable = "Use enter to fix the last point and ESC to cancel";
        ...
        private void RenderGradient(Surface surface, PdnRegion clipRegion, CompositingMode compositingMode,
            PointF startPointF, ColorPixelBase startColor, PointF endPointF, ColorPixelBase endColor)
        {
            ...
            for (int i = 0; i < Processor.LogicalCpuCount; ++i)
            {
                ...
                {
                    SciImage.Threading.ThreadPool.Global.QueueUserWorkItem(wc, BoxedConstants.GetInt32(i));
                }
            }

            SciImage.Threading.ThreadPool.Global.Drain();
        }

        private void RenderGradient()
        {
            ColorPixelBase startColor = AppEnvironment.PrimaryColor;
            ColorPixelBase endColor = AppEnvironment.SecondaryColor;
            ...
            if (this.shouldSwapColors)
            {
                if (AppEnvironment.GradientInfo.AlphaOnly)
                {
                    ...
                    byte startAlpha = startColor.alpha ;
                    startColor.alpha  = (byte)(255 - endColor.alpha );
                    endColor.alpha  = (byte)(255 - startAlpha);
                }
                else
                {
            ...
            Surface surface = ((Layer)DocumentWorkspace.ActiveLayer).Surface;
            PdnRegion clipRegion = DocumentWorkspace.Selection.CreateRegion();

            SaveRegion(clipRegion, clipRegion.GetBoundsInt(),surface.ColorPixelBase );
            ...
            string numberFormat;
            string unitsAbbreviation="pix";

            if (units != MeasurementUnit.Pixel)
            {
                string unitsAbbreviationName = units.ToString() + "s";
                numberFormat = "F2";
            }
            else
            {
        ...
        protected override void OnActivate()
        {
            ...
            this.startNub.Visible = false;
            this.startNub.Shape = SciImage.MoveNubRenderer.MoveNubShape.Circle;
            RendererList.Add(this.startNub, false);

            this.endNub = new MoveNubRenderer(RendererList);
            this.endNub.Visible = false;
            this.endNub.Shape = SciImage.MoveNubRenderer.MoveNubShape.Circle;
        ...
        public GradientTool(DocumentWorkspace documentWorkspace)
            : base(documentWorkspace,
                   StaticImage,
                   StaticName,
                   "Click and drag to set an gradient",
                   'g',
                   false,
