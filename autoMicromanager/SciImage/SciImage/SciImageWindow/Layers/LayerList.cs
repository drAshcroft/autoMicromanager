...
namespace SciImage
{
    ...
    /// re-render itself.
    /// This implementation also enforces that any contained blayer must be
    /// of the same dimensions as the document it is contained within.
    /// If you try to add a blayer that is the wrong size, an exception will
    /// be thrown.
    /// </summary>
    [Serializable]
        ...
        : ArrayList
    {
        ...
        private void CheckLayerSize(object value)
        {
            ...
            {
                throw new ArgumentException("Size of blayer does not match size of containing document");
            }
        }

        public override int Add(object value)
        {
            if (!(value is Layer) && value.GetType().IsAbstract==false  )
            {
        ...
        */
        ...
        public Layer  SetAtQuietly(int index, Layer newValue)
        {
            base.RemoveAt(index);
            CheckLayerSize(newValue);
            base.Insert(index, newValue);
