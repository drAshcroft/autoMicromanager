...
namespace SciImage
{
    public sealed class PaletteCollection
    {
        ...
        private Dictionary<string, ColorPixelBase[]> palettes; // maps from Name -> Palette
        ...
        public static ColorPixelBase DefaultColor
        {
            get
            {
        ...
        public static ColorPixelBase[] DefaultPalette
        {
            get
            {
                return
                    new ColorPixelBase[PaletteColorCount]
                    {
        ...
        public string[] PaletteNames
        {
            get
            {
                Dictionary<string, ColorPixelBase[]>.KeyCollection keyCollection = this.palettes.Keys;
        ...
        {
            this.palettes = new Dictionary<string, ColorPixelBase[]>();
        }
        ...
        private static bool ParseColor(string colorString, out ColorPixelBase color)
        {
            bool returnVal;

            try
            {
                color = (new ColorBgra()).ParseHexString(colorString);
                returnVal = true;
            }
        ...
        public static bool ParsePaletteLine(string line, out ColorPixelBase color)
        {
            color = DefaultColor;
        ...
        public static ColorPixelBase[] ParsePaletteString(string paletteString)
        {
            List<ColorPixelBase> palette = new List<ColorPixelBase>();
        ...
        public static ColorPixelBase[] LoadPalette(string palettePath)
        {
        ...
        private static string FormatColor(ColorPixelBase color)
        {
            return color.ToHexString();
        }

        public static string GetPaletteSaveString(ColorPixelBase[] palette)
        {
            StringWriter sw = new StringWriter();

            string header = "; comments in header";
            sw.WriteLine(header);

            foreach (ColorPixelBase color in palette)
            {
        ...
        public static void SavePalette(string palettePath, ColorPixelBase[] palette)
        {
        ...
        public void Load()
        {
            ...
            Dictionary<string, ColorPixelBase[]> newPalettes = new Dictionary<string, ColorPixelBase[]>();

            foreach (string pathName in pathNames)
            {
                ColorPixelBase[] palette = LoadPalette(pathName);
                ColorPixelBase[] goodPalette = EnsureValidPaletteSize(palette);
        ...
        public void Save()
        {
            ...
            {
                ColorPixelBase[] palette = this.palettes[paletteName];
                ColorPixelBase[] goodPalette = EnsureValidPaletteSize(palette);
                string fileName = Path.ChangeExtension(paletteName, PalettesFileExtension);
                string pathName = Path.Combine(palettesPath, fileName);
        ...
        public static ColorPixelBase[] EnsureValidPaletteSize(ColorPixelBase[] colors)
        {
            ColorPixelBase[] validPalette = new ColorPixelBase[PaletteColorCount];
        ...
        public ColorPixelBase[] Get(string name)
        {
            ...
            if (contains)
            {
                ColorPixelBase[] colors = this.palettes[existingKeyName];
                return (ColorPixelBase[])colors.Clone();
            }
            else
            {
                return null;
        ...
        public void AddOrUpdate(string name, ColorPixelBase[] colors)
        {
