...
namespace SciImage
{
    ...
    ///    the blayer's index instead, and access it via Workspace.Document.Layers[index].
    /// 2. The exception to #1 is if you are deleting a blayer. But you should use
    ///    DeleteLayerHistoryMemento for that. If you need to delete a blayer as part of a
    ...
    ///    to the blayer itself but we store the information needed to navigate to it.
    ///    The reasoning for this is made clear if you consider the following case. Assume you
    ///    are holding on to a blayer reference ("private Layer theLayer;"). Next, assume that
    ///    the blayer is deleted. Then the deletion is undone. The new blayer in memory is not
    ///    the blayer you have a reference to even though they hold the same data. Changes made
