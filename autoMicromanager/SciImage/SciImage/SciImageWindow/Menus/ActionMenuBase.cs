...
{
    public abstract class ActionMenuBase
        : PdnMenuItem
    {
        ...
        private List<Effects.Effect > MuttEffectList = new List<Effects.Effect >();
        ...
        private void HandleEffectException(AppWorkspace appWorkspace, Effect effect, Exception ex)
        {
            ...
            else
            {
                ...
                if (clickedTB == restartTB)
                {
                    ...
                    cawa.PerformAction(appWorkspace, null, -1);
        ...
        {
            List<string> AlreadyEnded = new List<string>();
            foreach (Effect E  in MuttEffectList )
            {
                if (E!=null)
                {
                    if (E.GetType().BaseType  == typeof(SciEffect))
                    {
                        AlreadyEnded.Add(((SciEffect)E).End(AlreadyEnded.ToArray()));
        ...
        private void AddEffectToMenu(Effect effect, bool withShortcut)
        {
            ...
            //System.Diagnostics.Debug.Print(effect.Name + " " + effect.GetType().ToString());
            if (effect.GetType().BaseType == typeof(SciEffect) )
            {
                if (((SciEffect)effect).CloneEffectToUse)
                    mi.Tag = effect;
                else
            ...
            mi.Name = effect.GetType().FullName; //"Effect(" + effect.GetType().FullName + ")";
            mi.Name.Replace("SciImage", "");
            mi.Name.Replace(".", "");
            mi.Name = effect.Name;
        ...
        private void AddEffectsToMenu()
        {
            ...
            Type[] effectArrayTypes = effectsCollection.EffectArrays;
            ...
            foreach (Type type in effectsCollection.EffectArrays )
            {
                System.Diagnostics.Debug.Print(type.ToString());
                try
                {
                        ...
                        ConstructorInfo ci = type.GetConstructor(Type.EmptyTypes);
                        IEffectTypeArray effectArray = (IEffectTypeArray)ci.Invoke(null);
                        Effect[] effects = effectArray.GetEffectTypeInstances();
                    ...
                    catch (Exception ex2)
                    {
                        // We don't want a DLL that can't be figured out to cause the app to crash
                        //continue;
                        AppWorkspace.ReportEffectLoadError(Triple.Create(type.Assembly, type, ex2));
        ...
        private void EffectMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //if (pmi.Tag.GetType().BaseType ==typeof(SciEffect )
        ...
        public void RunEffect(object  MenuEffect)
        {
            ...
            using (new PushNullToolMode(activeDW))
            {
                try
                {
                    ...
                    if (MenuEffect.GetType().BaseType  == typeof(System.Type))
                    {
                        effectType = (Type)MenuEffect;
                        effect = (Effect)Activator.CreateInstance((Type)MenuEffect);
                    }
                    else
                    {
                        effectType = MenuEffect.GetType();
                        effect = (SciEffect)((SciEffect)MenuEffect).Clone();
                    }
                    ...
                    if (!(effect.CheckForEffectFlags(EffectFlags.Configurable)))
                    {
                        RunUnConfigurableEffect(activeDW, effect,layer,selectedRegion,exception  );
                    }
                    else
                    {
                        RunConfigurableEffect(activeDW, effectType, effect, layer, selectedRegion,out exception,ref  newLastToken, ref resetDirtyValue  );
                    }
        ...
        private void RunUnConfigurableEffect(DocumentWorkspace activeDW, Effect effect, BitmapLayer layer, PdnRegion selectedRegion, Exception exception)
        {
            //Surface copy = activeDW.BorrowScratchSurface(this.GetType() + ".RunEffect() using scratch surface for non-configurable rendering");
            Surface copy=null;//=(Surface)  layer.Clone();
            try
            {
                using (new WaitCursorChanger(AppWorkspace))
                {
        ...
        private void RunConfigurableEffect(DocumentWorkspace activeDW, Type effectType, Effect effect, BitmapLayer layer, PdnRegion selectedRegion,out Exception exception,ref  EffectConfigToken newLastToken, ref bool resetDirtyValue)
        {
            ...
            try
            {
                ...
                using (EffectConfigDialog configDialog = effect.CreateConfigDialog())
                {
                    ...
                    if (effect.GetType().BaseType == typeof(SciEffect))
                    {
                        if (((SciEffect)effect).UseTileImageRender == false)
                        {
                            NumTiles = 1;
                            if (renderingThreadCount > NumTiles) renderingThreadCount = NumTiles;
                        }
                    }
                    else if (effect.CheckForEffectFlags(EffectFlags.RenderInOnePiece) == true)
                    {
                        NumTiles = 1;
                        if (renderingThreadCount > NumTiles) renderingThreadCount = NumTiles;
        ...
        private void StartingRenderingHandler(object sender, EventArgs e)
        {
            ...
            if (this.progressRegions == null)
            {
                this.progressRegions = new PdnRegion[TrueNumberofTiles ];
            }
                ...
                using (ProgressDialog aed = new ProgressDialog())
                {
                    ...
                    using (new WaitCursorChanger(AppWorkspace))
                    {
                        ...
                        try
                        {
                            ...
                            SciImage.Threading.ThreadPool.Global.QueueUserWorkItem(
