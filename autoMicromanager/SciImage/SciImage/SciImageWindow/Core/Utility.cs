...
namespace SciImage
{
    ...
    public sealed class Utility
    {
        ...
        public static int ColorDifference(ColorPixelBase a, ColorPixelBase b) 
        {
            return (int)Math.Ceiling(Math.Sqrt(ColorDifferenceSquared(a, b)));
        }

        public static int ColorDifferenceSquared(ColorPixelBase a, ColorPixelBase b) 
        {
            Int64  diffSq = 0, tmp;

            for (int i = 0; i < a.NumChannels; i++)
            {
                tmp = a.GetChannel(i) - b.GetChannel(i);
                diffSq += tmp * tmp;
            }
           

            return (int)diffSq / a.NumChannels ;
        }
