...
namespace SciImage
{
          ...
          IDeferredSerializable
    {
        ...
        /// <param name="srcOffset">The offset within src to copy bytes from.</param>
        /// <param name="length">The number of bytes to copy.</param>
        public static void CopyBlock(MemoryBlock dst, long dstOffset, IntPtr src, long srcOffset, long length)
        {
            ...
            void* dstPtr = (void*)((byte*)dst.VoidStar + dstOffset);
            void* srcPtr = (void*)((byte*)src + srcOffset);
            Memory.Copy(dstPtr, srcPtr, (ulong)length);
        }
        ...
        /// <summary>
        /// Creates a new MemoryBlock instance and allocates the requested number of bytes. and then copies the memory into the block
        /// </summary>
        /// <param name="Src" ></param>
        /// <param name="bytes"></param>
        public MemoryBlock(IntPtr Src, long bytes)
        {
            if (bytes <= 0)
            {
                throw new ArgumentOutOfRangeException("bytes", bytes, "Bytes must be greater than zero");
            ...
            this.voidStar = Allocate(bytes).ToPointer();
            this.valid = true;
            CopyBlock(this, 0, Src, 0, bytes);
        }

        public MemoryBlock(int width, int height)
            : this(width, height, 32) { }

        public MemoryBlock(int width, int height, int PixelSize)
        {
            if (width < 0 && height < 0)
            {
            ...
            this.mPixelSizeBPP = PixelSize;
            this.length = width * height * mPixelSizeBPP/8;//ColorPixelBase.SizeOf;
            this.parentBlock = null;
            this.voidStar = Allocate(width, height, mPixelSizeBPP , out this.bitmapHandle).ToPointer();
            this.valid = true;
        ...
        private void Dispose(bool disposing)
        {
            if (!disposed)
            {
                ...
                if (this.valid && parentBlock == null)
                {
                    ...
                    {
                        Memory.FreeBitmap(this.bitmapHandle, this.bitmapWidth, this.bitmapHeight,mPixelSizeBPP );
                    }
        ...
        private static IntPtr Allocate(int width, int height, int BPP, out IntPtr handle)
        {
            return Allocate(width, height,BPP, out handle, true);
        }

        private static IntPtr Allocate(int width, int height,int BPP, out IntPtr handle, bool allowRetry)
        {
            IntPtr block;
            
            try
            {
                block = Memory.AllocateBitmap(width, height,BPP, out handle);
            }
            ...
            catch (OutOfMemoryException)
            {
                if (allowRetry)
                {
                    ...
                    return Allocate(width, height,BPP, out handle, false);
                }
                else
                {
                    throw;
        ...
        private MemoryBlock(SerializationInfo info, StreamingContext context)
        {
            ...
            try
            {
                ...
                {
                    long bytes = (long)this.bitmapWidth * (long)this.bitmapHeight * (long)(this.mPixelSizeBPP/8) ;
        ...
        void IDeferredSerializable.FinishDeserialization(Stream input, DeferredFormatter context)
        {
            ...
            if (this.bitmapWidth != 0 && this.bitmapHeight != 0)
            {
                this.voidStar = Allocate(this.bitmapWidth, this.bitmapHeight,this.mPixelSizeBPP , out this.bitmapHandle).ToPointer();
                this.valid = true;
            }
            else
            {
            ...
            SciImage.Threading.ThreadPool threadPool = new SciImage.Threading.ThreadPool(Processor.LogicalCpuCount);
