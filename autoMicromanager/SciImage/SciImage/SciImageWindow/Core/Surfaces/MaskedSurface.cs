...
{
    /// <summary>
    /// Defines a RGB32_Surface that is irregularly shaped, defined by a Region.
          ...
          IDeserializationCallback
    {
        ...
        [NonSerialized]
        private static SciImage.Threading.ThreadPool threadPool = new SciImage.Threading.ThreadPool();

        /// <summary>
        /// Do not modify the RGB32_Surface. Treat it as immutable.
        /// </summary>
        ...
        public MaskedSurface(Surface source, PdnRegion roi)
        {
            ...
            this.surface = new Surface(boundsClipped.Size,surface.ColorPixelBase );
            this.surface.Clear( surface.ColorPixelBase.TranslateColor(  ColorBgra.FromUInt32(0x00ffffff) ));
        ...
        public MaskedSurface(Surface source, PdnGraphicsPath path)
        {
            ...
            {
                this.surface = new Surface(boundsRead.Size,source.ColorPixelBase );
        ...
        {
            threadPool = new SciImage.Threading.ThreadPool();
        ...
        private class DrawContext
        {
            ...
            public void DrawScansNearestNeighbor(object cpuNumberObj)
            {
                ...
                for (int i = cpuNumber; i < this.dstScans.Length; i += inc)
                {
                    ...
                    for (int dstY = dstRect.Top; dstY < dstRect.Bottom; ++dstY)
                    {
                        ...
                        if (dstY >= 0)
                        {
                            ...
                            // due to lack of enough precision, we will dive off the end of the RGB32_Surface
                            ...
                            //ColorPixelBase* dstPtr = dst.GetPointAddress(dstX, dstY);
                            //ColorPixelBase* dstPtrEnd = dstPtr + dstRect.Width;
                            ...
                            // Left side
                            for (int dstPtr=dstX; dstPtr < dstRect.Width ;dstPtr ++)
                            {
                                ...
                                dst.SetPoint(dstPtr,dstY , this.src.GetPoint(srcX, srcY) );
                           ...
                           /* //ColorPixelBase* startFastPtr = dstPtr;
                            //dstPtr = dstPtrEnd - 1;

                            // Right side
                            for (int dstPtr = dstRect.Left - 1; dstPtr >= dstRect.Right;dstPtr-- )
                            {
                            ...
                            //ColorPixelBase* endFastPtr = dstPtr;
                            ...
                            while (startFastPtr < endFastPtr)
                            {
                                ...
                                startFastPtr->Bgra = (unchecked(srcPtColX + (ColorPixelBase *)(((byte *)scan0) + (srcPtColY * stride))))->Bgra;
                ...
                for (int i = cpuNumber; i < this.dstScans.Length; i += inc)
                {
                    ...
                    for (int dstY = dstRect.Top; dstY < dstRect.Bottom; ++dstY)
                    {
                        ...
                        if (dstY >= 0)
                        {
                            ...
                            {
                                ColorPixelBase srcPixel = this.src.GetBilinearSample(srcPtCol.X, srcPtCol.Y,this.src.ColorPixelBase );
                                dst.SetPoint (dstX, dstY,srcPixel );
