...
namespace SciImage
{
        ...
        : UserControl
    {
        ...
        private void DrawGradient(Graphics g,ColorPixelBase RequestedColor)
        {
            ...
            {
                Surface gradientSurface = new Surface(gradientRect.Size,RequestedColor );
                ...
                using (RenderArgs ra = new RenderArgs(gradientSurface))
                {
                    ...
                    if (Orientation == Orientation.Horizontal)
                    {
                        for (int x = 0; x < gradientSurface.Width; ++x)
                        {
                            ...
                            {
                                ColorPixelBase src = gradientSurface.GetPoint(x, y);

                                // we are assuming that src.A = 255

                                int rd = ((rp * ap) + (src[2]  * (255 - ap))) / 255;
                                int gd = ((gp * ap) + (src[1]  * (255 - ap))) / 255;
                                int bd = ((bp * ap) + (src[0]  * (255 - ap))) / 255;

                                // TODO: proper alpha blending!
                                gradientSurface[x, y] = src.FromBgra((byte)bd, (byte)gd, (byte)rd, 255).ToInt32();
                            }
                        }
        ...
        {
            base.OnPaint (e);
            DrawGradient(e.Graphics,new ColorBgra());
        }

        protected override void OnPaintBackground(PaintEventArgs pevent)
        {
            DrawGradient(pevent.Graphics, new ColorBgra());
        }
