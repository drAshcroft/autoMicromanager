...
{
    public partial class Scilab_ScriptModule : UserControl, CoreDevices.UI.GUIDeviceControl
    {
        ...
        public void SetCore(EasyCore Ecore, string DeviceName)
        {
            ...
            ECore = Ecore;
            ECore.AddImageProcessor(DeviceName , new ImageProcessorStep(getImage));
            statConn = new StatConnector();
            statConn.Init("Scilab");
            statConn.AddGraphicsDevice("Graph1",(StatConnectorCommonLib.ISGFX) axGraphicsDevice1.GetOcx());
             
        }

        private StatConnector statConn;
        ...
        private void ExecuteCommand(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                s.Context.SetItem("Scilab_Conn", ContextItem.Variable, statConn );
                if (ic.InImage == null)
                {
                    long[,] a = new long[10, 10];
                    ic.InImage = CoreImage.CreateImageFromArray(a);
                }
                ic.OutImage = null;
