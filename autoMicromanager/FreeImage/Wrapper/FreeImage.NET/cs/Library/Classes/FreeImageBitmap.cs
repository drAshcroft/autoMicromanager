...
namespace FreeImageAPI
{
	...
	public class FreeImageBitmap : ICloneable, IDisposable, IEnumerable, ISerializable
	{
		...
		#endregion
        /// <summary>
        /// Returns the raw device independant bitmap.
        /// </summary>
        public FIBITMAP DIB
        {
            get { return dib; }
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="FreeImageBitmap"/> class
        /// based on the specified raw freeimage bitmap.  The string is to be left empty.
        /// </summary>
        /// <param name="dib"> The raw fibitmap</param>
        /// <param name="ingnorethis" >This parameter should not be used now </param>
        /// <exception cref="Exception">The operation failed.</exception>
        public FreeImageBitmap(FIBITMAP dib, string ingnorethis)
        {
            if (dib.IsNull)
            {
        ...
        //Brian's Modification.  Make a new constructor that allows a LUT to be installed
        private LUTControl LUT = null;

        /// <summary>
        /// Set the LUT control for working with grayscale images
        /// </summary>
        /// <param name="LUT">The LUT control that provides the grayscaling</param>
        /// <returns></returns>
        public LUTControl SetLUT(LUTControl LUT)
        {
            this.LUT = LUT;
            return (LUT);
        }
        ...
        /// multiplied by the width of the bitmap. The value passed to this parameter must
        /// be a multiple of four..</param>
        /// <param name="LUT">The control to make contrast possible with sci images</param>
        /// <param name="format">The PixelFormat enumeration for the new <see cref="FreeImageBitmap"/>.</param>
        /// <param name="scan0">Pointer to an array of bytes that contains the pixel data.</param>
        /// <exception cref="Exception">The operation failed.</exception>
        /// <exception cref="ArgumentException"><paramref name="format"/> is invalid.</exception>
        /// <exception cref="ArgumentOutOfRangeException">
        /// <paramref name="width"/> or <paramref name="height"/> are less or equal zero.</exception>
        public FreeImageBitmap(int width, int height, int stride, PixelFormat format, IntPtr scan0, LUTControl LUT )
        {
            this.LUT =LUT ;
            if (width <= 0)
            {
		...
		public static implicit operator Bitmap(FreeImageBitmap value)
		{
            ...
            //Brian's Modification:  Check if 16 grayscale
            if (value.LUT != null || (FreeImage.GetImageType(value.dib) != FREE_IMAGE_TYPE.FIT_BITMAP))
            {
                return FreeImage.GetBitmap(value.dib, value.LUT , true);
            }
            else
            {
        ...
        public long[,] GetBitmapArray()
        {
            ...
            PixelFormat format =FreeImage.GetPixelFormat(dib);

            int height = (int)FreeImage.GetHeight(dib);
            int width = (int)FreeImage.GetWidth(dib);
            int pitch = (int)FreeImage.GetPitch(dib);

            if ((format == PixelFormat.Undefined) && (FreeImage.GetBPP(dib) == 16u))
            {
                format = PixelFormat.Format16bppRgb555;
            }

            Bitmap tbuff = new Bitmap(width, height, format);
            long[,] OutArray = new long[width, height];
            BitmapData tdata;
            // Locking the complete bitmap in writeonly mode
            tdata = tbuff.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly, format);
            // Writing the bitmap data directly into the new created .NET bitmap.
            FreeImage.ConvertToRawBits(tdata.Scan0, dib, pitch, FreeImage.GetBPP(dib),
                FreeImage.GetRedMask(dib), FreeImage.GetGreenMask(dib), FreeImage.GetBlueMask(dib), true);
            // Unlock the bitmap
             
            //System.Diagnostics.Debug.Print("Image Start Convert");
             if ((FreeImage.GetBPP(dib) == 16u))
            {
                unsafe
                {
                    for (int y = 0; y < tdata.Height; y++)
                    {
                        ushort* inData = (ushort*)((byte*)tdata.Scan0 + (y * pitch));
            ...
            else
            {
                ...
                unsafe
                {
                    ...
                    for (int y = 0; y < tdata.Height; y++)
                    {
                        byte* inData = (byte*)tdata.Scan0 + (y * pitch);
        ...
        /// 
        /// <param name="histogram">Array of integers containing the histogram.</param>
        /// <param name="MaxValue">The maximum encountered in the data.</param>
        /// <param name="MinValue">The minimum encountered in the data.</param>
        /// <param name="MaxPossible">The maximum possible value.</param>
        /// <returns>Returns true on success, false on failure.</returns>
