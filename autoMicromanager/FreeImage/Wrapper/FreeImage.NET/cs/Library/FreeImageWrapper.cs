...
namespace FreeImageAPI
{
	...
	public static partial class FreeImage
	{
        ...
        /// <param name="LUT" >Used for conversion of 16bit grayscale.  >=is white</param>
        /// <returns>The converted .NET <see cref="System.Drawing.Bitmap"/>.</returns>
        /// <remarks>Copying metadata has been disabled until a proper way
        /// of reading and storing metadata in a .NET bitmap is found.</remarks>
        /// <exception cref="ArgumentNullException">
        ...
        /// <exception cref="ArgumentException">
        /// The image type of <paramref name="dib"/> is not FIT_BITMAP.</exception>
        internal static Bitmap GetBitmap(FIBITMAP dib,LUTControl LUT, bool copyMetadata)
        {
            if (dib.IsNull)
            {
            ...
            ConvertToRawBits(tdata.Scan0, dib, pitch, GetBPP(dib),
            ...
            if ((GetBPP(dib) == 16u))
            {
                ...
                double  slope = ( 65537/ (LUT.MaxContrast - LUT.MinContrast));
                double  b = -1 * LUT.MinContrast;
                ...
                unsafe
                {
                    ...
                    for (int y = 0; y < data.Height; y++)
                    {
                        ...
                        for (int x = 0; x < data.Width; x++)
                        {
                            ...
                            gray =(long )( slope * ( gray + b));

                            if (gray < 0) gray = 0;
                            else if (gray > 65536) gray =65536;

                            row[x * PixelSize] =(byte) (gray / 257);
                            row[x * PixelSize + 1] =(byte)( gray/257) ;
                            row[x * PixelSize + 2] =(byte)( gray /257);
            ...
            {
                double slope = (1 / (LUT.MaxContrast - LUT.MinContrast));
                double b = -1 * slope * LUT.MinContrast * 257;
                ...
                tbuff.SetResolution(GetResolutionX(dib), GetResolutionY(dib));
                // Apply the bitmaps resolution
           ...
           */
            ...
            System.Diagnostics.Debug.Print("exit Convert");
            return result;
        }
